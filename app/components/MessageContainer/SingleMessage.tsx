...
import { translateDates } from "~/utils/translateDates";
...
const SingleMessage = forwardRef<HTMLDivElement, Props>(
	({ message, isPrivate, isRead }, ref) => {
		...
		const align =
			message.author === "i-am-the-author"
				? "self-end"
		...
		useEffect(() => {
			if (socket) {
				socket.on("message_marked_read", ({ messageId, userChatId }) => {
